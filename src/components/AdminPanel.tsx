      ...
      if (res.ok) {
        ...
        if (data.success) {
          setEditingProduct(prev => prev ? {
            ...
            banglaDescription: data.descriptionBn,
            landingDescription: data.landingDescriptionEn,
            banglaLandingDescription: data.landingDescriptionBn
          } : null);
          setAiSuccessMessage(lang === "bn" ? "✨ এআই দিয়ে প্রোডাক্ট বিবরণী সফলভাবে জেনারেট করা হয়েছে!" : "✨ Rich SEO product descriptions successfully generated by AI!");
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm space-y-6">
          ...
          {isEditingProduct && (
            <form onSubmit={handleSaveProduct} className="bg-gray-50 border border-gray-100 p-5 rounded-2xl space-y-4">
              ...
              {/* AI COPYWRITING CARD */}
              <div className="bg-gradient-to-r from-indigo-50 to-indigo-100/50 border border-indigo-100 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="space-y-1">
                  <span className="inline-flex items-center gap-1 bg-indigo-200/60 border border-indigo-200 px-2.5 py-0.5 rounded-full text-[#3730a3] text-[9px] font-black uppercase tracking-wider">
                    <Sparkles className="w-2.5 h-2.5" /> {lang === "bn" ? "এআই রাইটার" : "Gemini AI Writer"}
                  </span>
                  <h5 className="text-xs font-black text-gray-800">
                    {lang === "bn" ? "অটোমেটেড ১-ক্লিক প্রোডাক্ট ডেসক্রিপশন জেনারেটর" : "Automated One-Click Copywriter"}
                  </h5>
                  <p className="text-[10px] text-gray-500 max-w-xl">
                    {lang === "bn"
                      ? "প্রোডাক্টের ইংরেজি নাম এবং ক্যাটাগরি লিখে নিচের বাটনে ক্লিক করুন। জেমিনি এআই চমৎকার এসিও-ফ্রেন্ডলি ডেসক্রিপশন লিখে দেবে।"
                      : "Type the Product Name (English), pick a Category, then click Generate to let Gemini write rich Bangla & English copy."}
                  </p>
                </div>
                <div className="flex flex-col items-end gap-2 shrink-0">
                  <button
                    type="button"
                    disabled={!editingProduct?.name || isGeneratingCopy}
                    onClick={handleGenerateAICopy}
                    className="w-full sm:w-auto bg-[#3730a3] hover:bg-[#4338ca] disabled:bg-gray-300 disabled:cursor-not-allowed text-white text-[10px] font-black uppercase tracking-wider px-4 py-2.5 rounded-xl flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-sm"
                  >
                    {isGeneratingCopy ? (
                      <>
                        <div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                        <span>{lang === "bn" ? "এআই দিয়ে লেখা হচ্ছে..." : "AI Writing..."}</span>
                      </>
                    ) : (
                      <>
                        <Sparkles className="w-3.5 h-3.5" />
                        <span>{lang === "bn" ? "এআই ডেসক্রিপশন তৈরি করুন" : "Generate Copy"}</span>
                      </>
                    )}
                  </button>
                  {aiSuccessMessage && (
